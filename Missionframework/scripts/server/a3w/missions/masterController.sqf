...
// Permanent controllers
{
	...
	missionNamespace setVariable [format ["%1Processor", _x],  compileFinal preprocessFileLineNumbers (format ["scripts\server\a3w\missions\%1Processor.sqf", _x])];

	// Start controller
	//[] execVM format ["scripts\server\a3w\missions\%1Controller.sqf", _x];
	[] spawn compileFinal preprocessFileLineNumbers format ["scripts\server\a3w\missions\%1Controller.sqf", _x];
	uiSleep 5;
} forEach _ctrlTypes;
	...
	for "_i" from 1 to _extraQuantity do
	{
		[_extraControllers, _extraNumber, _extraControllers select (_i - 1)] spawn
		{
			...
			while {true} do
			{
				...
				[_extraNumber, true] call compile preProcessFileLineNumbers format ["scripts\server\a3w\missions\%1Controller.sqf", _current select 0];
