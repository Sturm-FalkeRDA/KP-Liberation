...
_setupVars =
{
	...
	//_locArray = (((blufor_sectors select {_x select [0,8] == "capture_"}) apply {[_x, 200, markerText _x]}) call BIS_fnc_selectRandom);
	_locArray = selectRandom (blufor_sectors select {_x select [0,8] == "capture_"} apply {[_x, 200, markerText _x]});  
	_missionPos = (markerPos (_locArray select 0) vectorAdd [([[-100,0,100], 20] call F_getRND), ([[-100,0,100], 20] call F_getRND), 0]);
...
_setupObjects =
{
	...
	{ _x setVariable ["R3F_LOG_disabled", true, true] } forEach [_tent1, _chair1, _chair2, _cFire1];
