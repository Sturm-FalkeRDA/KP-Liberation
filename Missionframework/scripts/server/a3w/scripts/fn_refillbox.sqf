...
if (!isServer) exitWith {};
	...
	#define RANDOM_ODDS(ODDS) ([0,1] select (random 1 < (ODDS)))
	// between 0.0 and 1.0
...
switch (_boxType) do
{
	...
	case "mission_ACEMedical":
	{
		_boxItems =
		[
			// Item type, Item class(es), # of items, # of magazines per weapon
			//["itm", "NVGoggles", 5],
			["itm", "ACE_fieldDressing", RANDOM_BETWEEN(1,3)],
			["itm", "ACE_packingBandage", RANDOM_BETWEEN(1,3)],
			["itm", ["ACE_quikclot", "ACE_morphine", "ACE_tourniquet", "ACE_atropine", "ACE_elasticBandage"], RANDOM_BETWEEN(2,4)],
			["itm", ["ACE_personalAidKit", "ACE_surgicalKit", "ACE_salineIV", "ACE_salineIV_500", "ACE_salineIV_250", ["ACE_plasmaIV", "ACE_plasmaIV_500", "ACE_plasmaIV_250"], ["ACE_bloodIV_250", "ACE_bloodIV_500"]], RANDOM_BETWEEN(1,5)],
		];
	};
};
